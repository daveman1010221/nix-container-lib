...
let
  ...
  # ---------------------------------------------------------------------------
  # Build-time tool integration scripts
  #
  # atuin, starship, and direnv all provide `init nu` output that must be
  # sourced by nushell. Because `source` is parsed statically in nushell,
  # these files must exist at a stable path at parse time — /tmp won't work.
  # We run each tool at image build time and bake the output into /etc/nushell/.
  # ---------------------------------------------------------------------------
...
DIRENV_HOOK
  ...
  nuEnvText = ''
    ...
    # nushell parses `source` statically before env.nu executes.)
  '';
  ...
  nuConfigText = ''
    ...
    # Tool integrations — pre-generated at image build time
